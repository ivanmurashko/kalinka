...
#ifdef KLK_SOURCE
                /* Update KLK stat info */
                if (len > 0)
                    adapter->dvr.klkstat.count += len;                
#endif
...
#ifdef KLK_SOURCE
        event_base_set(a->klkbase, &a->dvr.stat.event);        
#endif // KLK_SOURCE
...
#ifdef KLK_SOURCE
        event_base_set(a->klkbase, &a->dvr.stucktimer);        
#endif // KLK_SOURCE
...
#ifdef KLK_SOURCE
        event_base_set(a->klkbase, &a->dvr.dvrevent);        
#endif // KLK_SOURCE
...
{
    event_del(&adapter->dvr.dvrevent);
    evtimer_del(&adapter->dvr.stat.event);    
    evtimer_del(&adapter->dvr.stucktimer);    

    close(adapter->dvr.fd);
    
    if (adapter->dvr.buffer.ptr)
    {        
        free(adapter->dvr.buffer.ptr);    
        adapter->dvr.buffer.ptr = NULL;
    }    
}
#endif
