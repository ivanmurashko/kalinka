...
struct klkstat_s 
{
    int count; ///< data count read
    struct timeval lastupdate; ///< last update time
};    
#endif //KLK_SOURCE
...
struct adapter_s {
...
#ifdef KLK_SOURCE
            struct klkstat_s klkstat; ///< klkstat info            
#endif
	} dvr;
	...
	struct {
		...
		struct {
			int	fd;
			int	type;
		} pidtable[PID_MAX+1+1];			/* Space for 0x200 pid structures */
	} dmx;
...
#ifdef KLK_SOURCE
    struct event_base* klkbase; ///< event base for multithreading    
    int klknewprog; ///< new prog flag    
#endif // KLK_SOURCE    
};
...
#ifdef KLK_SOURCE
void stream_deinit(struct stream_s *stream);
#endif // KLK_SOURCE
...
#ifdef KLK_SOURCE
void input_deinit(struct input_s *stream);
#endif // KLK_SOURCE
 ...
 */
#ifdef KLK_SOURCE
#define logwrite klk_log
#define LOG_ERROR KLKLOG_ERROR
//#define LOG_INFO KLKLOG_INFO
//#define LOG_DEBUG KLKLOG_DEBUG
#define LOG_EVENT KLKLOG_DEBUG
#define LOG_XTREME KLKLOG_XTREME
...
#ifdef KLK_SOURCE
void dvr_deinit(struct adapter_s *adapter);
#endif
...
#ifdef KLK_SOURCE
void dmx_deinit(struct adapter_s *adapter);
#endif
...
#ifdef KLK_SOURCE
void pmt_leave_pnr(struct adapter_s *a, unsigned int pnr, void *arg);
#endif //KLK_SOURCE
